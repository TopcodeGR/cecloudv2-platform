...
public class MeController {
    ...
    public UserInfoDto getMe(Authentication auth){
        ...
        if(auth instanceof JwtAuthenticationToken jwtAuthenticationToken){
            final String email  = (String) jwtAuthenticationToken.getTokenAttributes().getOrDefault(StandardClaimNames.EMAIL,"");

            final List<String> authorities = auth.getAuthorities().stream().map(GrantedAuthority::getAuthority).toList();

            final Long exp = Optional.ofNullable(jwtAuthenticationToken.getTokenAttributes().get(JwtClaimNames.EXP)).map(expClaim->{
    ...
    /**
     * @param username a unique identifier for the resource owner in the token (subclaim by default)
